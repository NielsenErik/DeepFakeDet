...
6.  Exact inference: log-density, marginals, MPE, partition function.
...
def _fit_leaves_with_jitter(model: nn.Module, X: torch.Tensor, jitter: float) -> None:
    """Shared fit_leaves implementation (DensityPC and SquaredPC): closed-form
    leaf init plus symmetry-breaking location jitter for scalar-mu leaves."""
    for m in model.modules():
        if isinstance(m, LeafNode):
            m.fit(X)
            if jitter > 0 and hasattr(m, "mu") and hasattr(m, "log_sigma"):
                with torch.no_grad():
...
def eval_log_marginal(
    root: nn.Module, x: torch.Tensor, marginalized: Iterable[int]
) -> torch.Tensor:
    """
    Exact marginal: leaves over marginalized features contribute their
    integral (log 1 = 0 for normalized leaves).  Requires smoothness and
    decomposability — exactly what validate_circuit checks.
    """
    marg = frozenset(marginalized)

    def _eval(node: nn.Module) -> torch.Tensor:
        if isinstance(node, LeafNode):
            if node.feature_idx in marg:
                return node.log_integral().to(x.dtype) + x.new_zeros(x.shape[0])
            return node.log_prob(x)
        if isinstance(node, ProductNode):
            return torch.stack([_eval(c) for c in node.child_nodes], dim=0).sum(dim=0)
        if isinstance(node, SumNode):
            stack = torch.stack([_eval(c) for c in node.child_nodes], dim=0)
            return torch.logsumexp(stack + node.log_weights().unsqueeze(-1), dim=0)
...
def log_partition(root: nn.Module) -> torch.Tensor:
    ...
    def _eval(node: nn.Module) -> torch.Tensor:
        if isinstance(node, LeafNode):
            return node.log_integral()
        if isinstance(node, ProductNode):
            return torch.stack([_eval(c) for c in node.child_nodes]).sum()
        if isinstance(node, SumNode):
            stack = torch.stack([_eval(c) for c in node.child_nodes])
            return torch.logsumexp(stack + node.log_weights(), dim=0)
        raise TypeError(f"Unknown circuit node type: {type(node)}")
...
def validate_structured_decomposability(root: nn.Module, vtree: VtreeNode) -> None:
    """
    Property 4: every product node splits its scope exactly as the given
    vtree prescribes.  Requires binary product nodes.
    """
    allowed = {
    ...
    def _check(node: nn.Module) -> None:
        ...
        if isinstance(node, (ProductNode, _SignedProduct)):
            assert len(node.child_nodes) == 2, (
                "Structured decomposability check requires binary product nodes; "
                f"found one with {len(node.child_nodes)} children"
            )
            partition = frozenset({
                _compute_scope(node.child_nodes[0], cache, False, False),
                _compute_scope(node.child_nodes[1], cache, False, False),
            })
...
class SquaredPC(nn.Module):
    ...
    def _signed_forward(self, node: nn.Module, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        if isinstance(node, LeafNode):
            la = node.log_prob(x)
            return la, torch.ones_like(la)
        if isinstance(node, _SignedProduct):
            la_l, s_l = self._signed_forward(node.child_nodes[0], x)
            la_r, s_r = self._signed_forward(node.child_nodes[1], x)
            return la_l + la_r, s_l * s_r
        if isinstance(node, _SignedSum):
            las, ss = zip(*[self._signed_forward(c, x) for c in node.child_nodes])
            w = node.weights
            la = torch.stack(las, dim=0) + torch.log(w.abs() + 1e-38).unsqueeze(-1)
            s = torch.stack(ss, dim=0) * torch.sign(w).unsqueeze(-1)
            return _signed_lse(la, s, dim=0)
    ...
    def _pair_eval(
        ...
        if isinstance(n, LeafNode) and isinstance(m, LeafNode):
            ...
            la_l, s_l = self._pair_eval(n.child_nodes[0], m.child_nodes[0], x, marg, cache)
            la_r, s_r = self._pair_eval(n.child_nodes[1], m.child_nodes[1], x, marg, cache)
    ...
    def log_partition(self) -> torch.Tensor:
        ...
        scope = frozenset(vtree_leaves(self.vtree))
